...
export default class PlaylistPicker extends React.Component {
	...
	/**
	 * Checks if the url holds the correct format for fetching a Spotify playlist, but not if it exists
	 */
	validUrl(url) {
		...
		return (
			<Row className="show-grid" >
				...
				<Col xs={12} sm={6} md={6} >
					...
					<Select id="selectPlaylist" options={this.state.playlists} value={this.state.value} onChange={this.choosePlaylist} valueKey="id" labelKey="name" />
				</Col>
				<Col xs={12} sm={6} md={6} >
					<h2>Paste link</h2>
					...
					<form>
						<FormGroup controlId="formBasicText" >
							<FormControl type="text" onChange={this.getPlaylistFromLink} placeholder="Paste URL from Spotify" />
						</FormGroup>
					</form>
				</Col>
			</Row>
